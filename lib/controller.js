class Controller {
  ...
  findRegexCompany (req, res, next) {
    return this.facade.findRegexCompany(req.params.company)
      .then(collection => res.status(200).json(collection))
      .catch(err => next(err))
  }
  ...
  aggregateChange (req, res, next) {
    return this.facade.aggregateChange(req.params.key,req.params.value)
      .then(doc => res.status(200).json(doc))
      .catch(err => next(err))
  }

  aggregateAvgVolume (req, res, next) {
    return this.facade.aggregateAvgVolume(req.params.key,req.params.value)
      .then(doc => res.status(200).json(doc))
      .catch(err => next(err))
  }

  aggregateROI (req, res, next) {
    return this.facade.aggregateROI(req.params.key,req.params.value)
      .then(doc => res.status(200).json(doc))
      .catch(err => next(err))
  }
